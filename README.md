# Java Test Project
This project explore basic concepts of java, the project is a calculator and there are three forms to use:

## First way
You can use the command line pointing to the project

## Second way
Just run the project and see the result, there are default values so you can run from your IDE

## Thrid way
This explication is form intellij IDE but apply to any IDE: 
Go to Run options in the top of your IDE, select Edit Configurations, in the input "Program arguments" write "interactive", click ok and run the project in the IDE

